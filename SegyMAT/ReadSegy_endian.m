...
function [Data,SegyTraceHeaders,SegyHeader,HeaderInfo]=ReadSegy_endian(filename,varargin);
...
while (~(ftell(segyid)>=DataEnd))
    ...
    elseif (SegyHeader.FixedLengthTraceFlag==0)
        ...
        SingleSegyTraceHeaders=GetSegyTraceHeader_endian(segyid,TraceStart,Format,SegyHeader.ns,[]);
